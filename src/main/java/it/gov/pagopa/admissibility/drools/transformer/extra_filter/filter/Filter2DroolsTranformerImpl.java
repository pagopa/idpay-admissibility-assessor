...
@Service
public class Filter2DroolsTranformerImpl implements Filter2DroolsTranformer {
    private final List<OperationValueBuilder> operationValueBuilders;

    @Autowired
    public Filter2DroolsTranformerImpl(InstanceOfOpValueBuilder instanceOfOpValueBuilder, ScalarOpValueBuilder scalarOpValueBuilder, InOpValueBuilder inOpValueBuilder) {
        this.operationValueBuilders = new ArrayList<>(3);
        this.operationValueBuilders.add(scalarOpValueBuilder);
        this.operationValueBuilders.add(instanceOfOpValueBuilder);
        this.operationValueBuilders.add(inOpValueBuilder);
    }
    ...
    private String transcodeFilterOp(FilterOperator filterOperator, Class<?> fieldType, @NotNull String value) {
        switch (filterOperator) {
            case EQ:
                if(Collection.class.isAssignableFrom(fieldType) && !value.startsWith("(") && !value.endsWith(")")){
                    return "contains";
                } else {
                    return "==";
                }
